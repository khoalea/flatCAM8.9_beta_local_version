...
# http://flatcam.org                                       #
# Makera Carvera Air variant of grbl_11.py                 #
# Uses the Carvera ATC toolchange sequence (single          #
# 'M6 T<n>' line, no manual-collet pause) instead of the    #
# generic GRBL 'T../M6/M0' sequence.                        #
# MIT Licence                                               #
# ##########################################################
...
class Makera_Air(PreProc):
    ...
    def start_code(self, p):
        ...
        gcode = '(This preprocessor is used with a Makera Carvera Air machine.)\n\n'
    ...
    def toolchange_code(self, p):
        # Bare "M6 T<n>" is the documented Carvera Air ATC sequence (Fusion360's
        # Carvera.cps "Carvera Air Tool Changes" mode writes exactly this, nothing
        # more). M6 retracts, swaps the tool and auto-calibrates its length (TLO)
        # entirely in firmware. Adding a manual G28/M600 before it moves Z to a
        # position the calibration probe doesn't expect, which is what was tripping
        # "Soft Endstop Z was exceeded" during the tool-length calibration step.
        f_plunge = p.f_plunge
...
""".format(tool=int(p.tool), t_drills=no_drills, toolC=toolC_formatted)
        else:
            gcode = """
M05
M6 T{tool}
(MSG, Change to Tool Dia = {toolC})
""".format(tool=int(p.tool), toolC=toolC_formatted)
